...
import { installWorkflowProfileReferenceGuards } from './0070_polymorphic_common_tp'
...
export const up = async (db: Kysely<Database>): Promise<void> => {
  ...
  await db.schema
    ...
    .addColumn('egcs_fc_applicantrecipientsubtype', 'bigint', col =>
      col.references('Agency_Applicant_Recipient_Subtype.id').onDelete('restrict')
    )
    .addColumn('_deleted', 'boolean', col => col.defaultTo(false).notNull())
    .execute()
...
export const down = async (db: Kysely<Database>): Promise<void> => {
  for (const [table, trigger] of [
    ...
    ['Funding_Case_Agreement_Applicant_Recipient', 'guard_agreement_proponent_type'],
    ['Transfer_Payment_Stream', 'guard_stream_proponent_types'],
    ['Transfer_Payment_Profile', 'guard_program_proponent_types'],
    ['Funding_Case_Agreement_Profile', 'guard_agreement_stream_proponent_types'],
    ['Transfer_Payment_Stream_Eligible_Recipient', 'guard_eligible_proponent_types'],
    ['Agency_Applicant_Recipient_Subtype', 'guard_referenced_proponent_subtype']
  ]) {
    await sql.raw(`DROP TRIGGER IF EXISTS ${trigger} ON "${table}"`).execute(db)
  }
  for (const signature of [
    'guard_referenced_proponent_subtype()', 'guard_stream_proponent_types()',
    'guard_agreement_proponent_type()', 'guard_future_proponent_type()',
    'validate_proponent_type_choice(bigint, bigint, bigint)', 'validate_stream_proponent_types(bigint)'
  ]) await sql.raw(`DROP FUNCTION IF EXISTS ${signature}`).execute(db)

...
export const replaceWorkflowConditions = async (
  trx: Transaction<Database>, agencyId: string, memberId: string, conditions: WorkflowMemberCondition[]
): Promise<boolean> => {
  ...
  const fields = await readAgencyCustomFieldDefinitions(trx, agencyId)
  if (custom.some(condition => {
    const field = fields.find(candidate => candidate.id === condition.fieldId)
    return field?.egcs_ay_kind !== 'relational' || !field.egcs_ay_discriminator
      || condition.optionIds.some(id => !field.options.some(option => option.id === id && option.egcs_ay_active))
  })) return false

import { AgencyAddressTypeSchema } from '~~/shared/types/schemas'
...
export default defineEventHandler(async event => {
  ...
  const body = await readValidatedBodyI18n(event, AgencyAddressTypeSchema.partial())
  ...
  try {
    result = await withActiveAgencyMutationTransaction(event, agencyId, async trx => await trx
      .updateTable('Agency_Address_Type')
      .set(body)
      .where('id', '=', id)
  ...
  if (!result) return await notFound(event, 'ADDRESS_TYPE_NOT_FOUND', 'apiErrors.agency.address_type_not_found')
  return result
})

...
export const ReviewSetupListQuerySchema = PaginationSchema.extend({
  ...
  publicationState: z.enum(['draft', 'published', 'retired'], { error: 'validation.invalid_selection' }).optional(),
...
const applyReviewSetupTypeFilter = async (
  ...
  const reviewSetIds = await getReviewSetupIdsByType(db, agencyId, reviewType)
...
export default defineEventHandler(async event => {
  ...
  const { page, limit, search, reviewType, entityType, publicationState } = await readReviewSetupListQuery(event)

  const offset = (page - 1) * limit

  let baseQuery = createReviewSetupBaseQuery(db, agencyId)
  if (entityType) {
    baseQuery = baseQuery.where('Common_Review_Set_Setup.egcs_cn_entitytype', '=', entityType)
  }
  if (publicationState) {
    baseQuery = baseQuery.where('Common_Publication.egcs_cn_state', '=', publicationState)
  }
  const typeFilterResult = await applyReviewSetupTypeFilter(db, agencyId, baseQuery, reviewType)

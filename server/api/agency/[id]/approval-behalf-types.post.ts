...
export default defineEventHandler(async event => {
  ...
  const validated = await readValidatedBodyI18n(event, AgencyApprovalBehalfTypeSchema)
  try {
    return await withActiveAgencyMutationTransaction(event, agencyId, async trx => {
      return await trx.insertInto('Agency_Approval_Behalf_Type')
        .values({
          egcs_ay_organizationagency: agencyId,
          egcs_ay_name_en: validated.egcs_ay_name_en,
          egcs_ay_name_fr: validated.egcs_ay_name_fr,
          egcs_ay_require_actual: validated.egcs_ay_require_actual
        })
        .returningAll()

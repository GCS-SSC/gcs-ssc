...
const APPLICANT_RECIPIENT_SUBTYPE_STATUS: ApplicantRecipientSubtypeStatus[] = [
  'aboriginalrecipients',
  'forprofitorganizations',
  'government',
  'internationalnongov',
...
const isApplicantRecipientSubtypeStatus = (value: string): value is ApplicantRecipientSubtypeStatus => {
  return APPLICANT_RECIPIENT_SUBTYPE_STATUS.includes(value as ApplicantRecipientSubtypeStatus)
}
...
export default defineEventHandler(async event => {
  ...
  return await withActiveAgencyReadTransaction(event, agencyId, async trx => {
    ...
    if (status && status !== 'all') {
      if (!isApplicantRecipientSubtypeStatus(status)) {
        return await badRequest(event, 'INVALID_STATUS', 'apiErrors.request.invalid_status')
      }
      baseQuery = baseQuery.where('egcs_ay_applicantrecipienttype', '=', status)
    }
    ...
    if (search) {
      ...
      baseQuery = baseQuery.where(eb =>
        eb.or([
          ...
          eb('egcs_ay_description_en', 'ilike', `%${escapedSearch}%`),
          eb('egcs_ay_description_fr', 'ilike', `%${escapedSearch}%`)
        ])
      )
    }

...
export default defineEventHandler(async event => {
  ...
  const validated = await readValidatedBodyI18n(event, AgencyAgreementTypeSchema)
  try {
    return await withActiveAgencyMutationTransaction(event, agencyId, async trx => {
      return await trx.insertInto('Agency_Agreement_Type')
        .values({
          egcs_ay_organizationagency: agencyId,
          egcs_ay_agreementtype: validated.egcs_ay_agreementtype,

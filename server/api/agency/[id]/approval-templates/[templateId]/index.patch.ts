...
export default defineEventHandler(async event => {
  ...
  const scopeContext = await authorizeApprovalTemplateById(event, 'update', agencyId, templateId)
  ...
  const saved = await executeApprovalTemplateScopeWrite(event, 'update', scopeContext, async trx => {
    ...
    if (!currentScopeContext || currentScopeContext.agencyId !== agencyId || !existing) {

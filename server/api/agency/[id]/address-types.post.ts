...
export default defineEventHandler(async event => {
  ...
  const validated = await readValidatedBodyI18n(event, AgencyAddressTypeSchema)
  try {
    return await withActiveAgencyMutationTransaction(event, agencyId, async trx => {
      return await trx
        ...
        .values({
          ...
          egcs_ay_typename_en: validated.egcs_ay_typename_en,
          egcs_ay_typename_fr: validated.egcs_ay_typename_fr
        })
        .returningAll()

...
export default defineEventHandler(async event => {
  ...
  const body = await readValidatedBodyI18n(event, AgencyCostCategoryLineItemSchema.partial())
  ...
  let result
  try {
    result = await withActiveAgencyCostCategoryMutationTransaction(event, agencyId, costCategoryId, async trx => await trx

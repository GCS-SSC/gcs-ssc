import { AgencyMonitorTypeSchema } from '~~/shared/types/schemas'
...
export default defineEventHandler(async event => {
  ...
  const body = await readValidatedBodyI18n(event, AgencyMonitorTypeSchema)
  try {
    return await withActiveAgencyMutationTransaction(event, agencyId, async trx => await trx
      .insertInto('Agency_Monitor_Type').values({ ...body, egcs_ay_organizationagency: agencyId })

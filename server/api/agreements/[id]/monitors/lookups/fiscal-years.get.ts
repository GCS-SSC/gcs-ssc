...
export default defineEventHandler(async event => {
  ...
  const baseQuery = db
    ...
    .$if(search.length > 0, qb => qb.where('egcs_ay_fiscalyeardisplay', 'ilike', `%${search}%`))

  const [items, countResult] = await Promise.all([
    baseQuery.select(['id', 'egcs_ay_fiscalyeardisplay as label_en', 'egcs_ay_fiscalyeardisplay as label_fr']).orderBy('egcs_ay_fiscalyeardisplay', 'asc').limit(limit).offset((page - 1) * limit).execute(),

import { FundingCaseAgreementMonitorPromisingPracticeCreateSchema } from '~~/shared/types/schemas'
import { executeAgreementMonitorMutation, prepareAgreementMonitorRoute } from '~~/server/utils/agreement-monitor'

export default defineEventHandler(async event => {
  const validated = await readValidatedBodyI18n(event, FundingCaseAgreementMonitorPromisingPracticeCreateSchema)
  ...
  return await executeAgreementMonitorMutation(event, db, agreementId, agreementContext, validated.egcs_fc_fundingagreementmonitor, async trx => {
    return await trx.insertInto('Funding_Case_Agreement_Monitor_Promising_Practice').values(validated).returningAll().executeTakeFirstOrThrow()
  }, { action: 'create' })
})

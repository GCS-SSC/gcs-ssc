...
const AgencyFilterSchema = z.coerce.string().transform((value, ctx) => {
  if (value === '') return value
  const match = /^([+-]?)([0-9]+)$/.exec(value.replace(/^[ \t\n\r\f\v]+|[ \t\n\r\f\v]+$/g, ''))
  if (match) {
    const digits = match[2]!.replace(/^0+(?=[0-9])/, '')
    const normalized = digits === '0' ? '0' : `${match[1] === '-' ? '-' : ''}${digits}`
    if (isCanonicalPostgresBigintText(normalized)) return normalized
  }
  ctx.addIssue({ code: 'custom', message: 'validation.invalid_selection' })
  return z.NEVER
})
...
export default defineEventHandler(async event => {
  return await executeFreshReadSnapshot(event, async db => {
    ...
    const permissions = await resolveAgreementPageMutationPermissions(context, items.map(item => ({
    ...
    const itemsWithPermissions = itemsWithState.map(item => {
      const permission = permissions.get(String(item.id))
      return {
        ...item,
        can_update: permission?.canUpdate ?? false,
        can_delete: permission?.canDelete ?? false
      }
    })

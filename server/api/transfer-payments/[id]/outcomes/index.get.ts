...
export default defineEventHandler(async event => {
  ...
  if (search) {
    baseQuery = baseQuery.where(eb =>
      eb.or([eb('Transfer_Payment_Outcome.egcs_tp_name_en', 'ilike', `%${escapeLikePattern(search)}%`), eb('Transfer_Payment_Outcome.egcs_tp_name_fr', 'ilike', `%${escapeLikePattern(search)}%`)])
    )
  }

  const [items, countResult] = await Promise.all([
    baseQuery.selectAll('Transfer_Payment_Outcome').orderBy('Transfer_Payment_Outcome.id', 'asc').limit(limit).offset(offset).execute(),

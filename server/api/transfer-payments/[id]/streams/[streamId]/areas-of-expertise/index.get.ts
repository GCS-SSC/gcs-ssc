...
export default defineEventHandler(async event => {
  ...
  let baseQuery = db
    ...
    .innerJoin('Transfer_Payment_Stream', 'Transfer_Payment_Stream.id', 'Transfer_Payment_Stream_Area_of_Expertise.egcs_tp_transferpaymentstream')
    ...
    .where('Transfer_Payment_Stream_Area_of_Expertise.egcs_tp_transferpaymentstream', '=', streamId)
    .where('Transfer_Payment_Stream.egcs_tp_transferpaymentprofile', '=', profileId)
    .where('Transfer_Payment_Stream_Area_of_Expertise._deleted', '=', false)
  ...
  if (search) {
    baseQuery = baseQuery.where(eb =>
      eb.or([eb('Transfer_Payment_Stream_Area_of_Expertise.egcs_tp_name_en', 'ilike', `%${escapeLikePattern(search)}%`), eb('Transfer_Payment_Stream_Area_of_Expertise.egcs_tp_name_fr', 'ilike', `%${escapeLikePattern(search)}%`)])
    )
  }

  const [items, countResult, statsResult] = await Promise.all([
    baseQuery.selectAll('Transfer_Payment_Stream_Area_of_Expertise').orderBy('Transfer_Payment_Stream_Area_of_Expertise.id', 'asc').limit(limit).offset(offset).execute(),
    baseQuery.select(eb => eb.fn.count('Transfer_Payment_Stream_Area_of_Expertise.id').as('total')).executeTakeFirst(),
    db
      .selectFrom('Transfer_Payment_Stream_Area_of_Expertise')
      .innerJoin('Transfer_Payment_Stream', 'Transfer_Payment_Stream.id', 'Transfer_Payment_Stream_Area_of_Expertise.egcs_tp_transferpaymentstream')
      .innerJoin('Transfer_Payment_Profile', 'Transfer_Payment_Profile.id', 'Transfer_Payment_Stream.egcs_tp_transferpaymentprofile')
      ...
      .where('Transfer_Payment_Profile._deleted', '=', false)
      .where('Agency_Profile._deleted', '=', false)
      .select(eb => eb.fn.count('Transfer_Payment_Stream_Area_of_Expertise.id').as('total'))
      .executeTakeFirst()
  ])

...
import {
  buildChecklistReviewSchemaDefinition,
  fetchChecklistReviewSchemaForAgency,
  getEffectiveChecklistDefinition
} from '~~/server/utils/transfer-payment-checklist-schemas'
...
export default defineEventHandler(async event => {
  ...
  return await executeFreshAuthorizedTransferPaymentStreamWrite(
    event, db, profileId, context.agencyId, streamId, 'update', async (trx, freshContext) => {
      const schema = await fetchChecklistReviewSchemaForAgency(trx, freshContext.agencyId, schemaId, true)
      ...
      return await publishDefinition(trx, {
        ...
        kind: 'review_schema',
        definition: buildChecklistReviewSchemaDefinition(schema, getEffectiveChecklistDefinition(schema)),
        actorId: actor.id
      })
    }
  )

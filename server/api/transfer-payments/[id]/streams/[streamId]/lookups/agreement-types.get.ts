...
const AgreementTypeLookupQuerySchema = PaginationSchema.extend({
...
export default defineEventHandler(async event => {
  ...
  return await executeFreshReadSnapshot(event, async db => {
    ...
    const { page, limit, search } = await getValidatedQueryI18n(event, AgreementTypeLookupQuerySchema)
    let baseQuery = db.selectFrom('Agency_Agreement_Type')
    ...
    const [items, count] = await Promise.all([
      baseQuery.select(['id', 'egcs_ay_organizationagency', 'egcs_ay_agreementtype', 'egcs_ay_name_en', 'egcs_ay_name_fr', '_deleted'])

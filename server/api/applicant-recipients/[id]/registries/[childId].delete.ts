...
export default defineEventHandler(async event => {
  ...
  return await executeFreshAuthorizedApplicantRecipientWrite(event, db, applicantRecipientId, 'delete', async trx => {
    const existing = await assertApplicantRecipientChildExists(event, trx.selectFrom('Applicant_Recipient_Registry')
      ...
      .select('id').executeTakeFirst(), ...APPLICANT_RECIPIENT_CHILD_ERROR_KEYS.registryNotFound)
    if (!existing || typeof existing !== 'object' || !('id' in existing)) return existing
    await trx.updateTable('Applicant_Recipient_Registry').set({ _deleted: true }).where('id', '=', childId)
      .where('egcs_ar_applicantrecipient', '=', applicantRecipientId).where('_deleted', '=', false).execute()
    return { success: true }
  })
})

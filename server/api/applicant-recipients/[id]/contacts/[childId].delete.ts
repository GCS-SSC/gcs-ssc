    ...
    async trx => {
      const existing = await assertApplicantRecipientChildExists(
        ...
        trx
          ...
          .select([
            'Applicant_Recipient_Contact.id as id',
            'Applicant_Recipient_Contact.egcs_ar_contact as egcs_ar_contact'
          ])
      ...
      await trx
        .updateTable('Applicant_Recipient_Contact')
        .set({ _deleted: true })
        .where('id', '=', childId)
      ...
      const contactIsStillReferenced = await hasOtherActiveCommonContactReferences(
        trx,
        existing.egcs_ar_contact,
        null
      )
      if (!contactIsStillReferenced) {
        await trx

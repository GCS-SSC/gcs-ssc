...
import { readDemoImage } from '../deployment/demo-image.js'
...
export default defineRailway((ctx) => {
  ...
  const gcsSsc = service('gcs-ssc', {
    source: demoImage
      ? image(demoImage, { autoUpdates: { type: 'disabled' } })
      : github('GCS-SSC/gcs-ssc', { branch: 'main' }),
    build: demoImage ? undefined : { builder: 'DOCKERFILE', dockerfilePath: 'Dockerfile' },
  ...
  // The SDK prunes nulls from constructor inputs. Set explicit clearing values
  // on the final node when promoting an existing Dockerfile service to an image.
  if (demoImage) gcsSsc.build = { builder: null, dockerfilePath: null }

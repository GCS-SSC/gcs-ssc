...
const waitForProductionMigrations = async (
  ...
  const expectedLine = 'migration "0014_program_terms_links" was executed successfully'
...
const waitForDatabaseUrlPrecedence = async (
  ...
  while (Date.now() < deadline) {
    const output = artifact.output()
    if (output.includes('migration "0014_program_terms_links" was executed successfully')) {

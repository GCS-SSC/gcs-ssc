...
const waitForProductionMigrations = async (
  ...
  const expectedLine = 'migration "0017_custom_field_namespaces" was executed successfully'
...
const waitForDatabaseUrlPrecedence = async (
  ...
  while (Date.now() < deadline) {
    const output = artifact.output()
    if (output.includes('migration "0017_custom_field_namespaces" was executed successfully')) {

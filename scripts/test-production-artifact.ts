...
const waitForProductionMigrations = async (
  ...
  const expectedLine = 'migration "0170_funding_case_group_assignment" was executed successfully'
...
const waitForDatabaseUrlPrecedence = async (
  ...
  while (Date.now() < deadline) {
    const output = artifact.output()
    if (output.includes('migration "0170_funding_case_group_assignment" was executed successfully')) {

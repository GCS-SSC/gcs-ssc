...
const { entityType, readonly = false, customFields, choices, loadError = false } = defineProps<{
  entityType: string
  readonly?: boolean
  customFields: AgreementCustomFieldDefinition[]
...
const fields = computed(() => customFields.filter(field => field.egcs_tp_discriminator && field.egcs_tp_kind === 'relational' && (field.egcs_tp_active || selectedOptionIds(field.id).length > 0)))
...
const sources: Source[] = ['agreement_subtype', 'further_distribution', 'holdback_basis', 'proponent_type']
...
const addCondition = (source: Source | null) => {
  ...
  const condition: AgreementProfileCondition = source === 'further_distribution'
    ? { source, value: false }
    : source === 'proponent_type' ? { source, quantifier: 'any', optionIds: [] } : { source, optionIds: [] }
...
const badges = computed(() => model.value.map(condition => {
  ...
  return {
    ...
    option: condition.source === 'proponent_type' ? t(`workflow.conditions.${condition.quantifier}`) : null
  }
}))
</script>
    ...
    <template v-if="entityType !== 'applicantrecipient'">
      ...
      <div v-for="{ condition, index } in profileRows" :key="condition.source" class="space-y-3">
        ...
        <template v-else>
          ...
          <UFormField v-if="condition.source === 'proponent_type'" :name="`conditions.${index}.quantifier`" :label="t('workflow.conditions.quantifier')" required>

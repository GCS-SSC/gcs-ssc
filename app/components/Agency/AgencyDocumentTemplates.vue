...
const {
  ...
  refresh,
  status
} = useResourceTable<AgencyDocumentTemplateItem>({
  fetchUrl: computed(() => `/api/agency/${agencyId}/document-templates`)
})

const columns: TableColumnInput<AgencyDocumentTemplateItem>[] = [
...
const templateModal = useCrudModal<AgencyDocumentTemplateItem, TemplateFormState>({
  createState: () => ({
    egcs_ay_entitytype: 'fundingcaseagreement',
    egcs_ay_templatekind: 'docx',
    egcs_ay_outputformats: ['docx', 'pdf'],
    egcs_ay_active: true,
    fileEn: null,
    fileFr: null
  }),
...
const validateMetadata = createValidator(AgencyDocumentTemplateCreateSchema)
...
const fileAccept = computed(() => selectedTemplate.value?.egcs_ay_templatekind === 'html' ? '.html,.htm' : '.docx')
const compatibleOutputFormats: Record<'docx' | 'html', TransferPaymentDocumentTemplateOutputFormat[]> = {
  docx: ['docx', 'pdf'],
  html: ['html', 'pdf']
}
const { getBilingualValue } = useBilingualValue()
const getTemplateActionTarget = (template: AgencyDocumentTemplateItem) =>
  `${getBilingualValue(template, 'egcs_ay_name', String(template.id))} [${template.id}]`
const getDownloadActionName = (template: AgencyDocumentTemplateItem, language: 'eng' | 'fra') =>
  `${t('common.download')}: ${getTemplateActionTarget(template)} (${t(`enums.language_preference.${language}`)})`

const getTemplateDownloadUrl = (template: AgencyDocumentTemplateItem, language: 'eng' | 'fra') => {
  return `/api/agency/${agencyId}/document-templates/${template.id}/download?language=${language}`
}

const canSelectOutputFormat = (format: TransferPaymentDocumentTemplateOutputFormat) => {
  const kind = selectedTemplate.value?.egcs_ay_templatekind
...
watch(() => selectedTemplate.value?.egcs_ay_templatekind, kind => {
  if (!selectedTemplate.value || selectedTemplate.value.id || !kind) return
  selectedTemplate.value.egcs_ay_outputformats = [...compatibleOutputFormats[kind]]
  clearSelectedTemplateFiles()
}, { flush: 'sync' })
watch(() => agencyId, () => templateModal.close())
...
const saveTemplate = async () => {
  ...
  const request = getDocumentTemplateSaveRequest(agencyId, item)
...
const deleteTemplate = async (row: AgencyDocumentTemplateItem) => {
  try {
    const ok = await confirmDeleteRequest(`/api/agency/${agencyId}/document-templates/${row.id}`)
    if (!ok) return
...
<template>
  <div class="space-y-6">
    <CommonResourceLayoutCard
      ...
      :button-label="canCreate ? t('common.add') : undefined"
      ...
      <template #egcs_ay_entitytype-cell="{ row }">
        <CommonEntityTypeBadge :type="row.original.egcs_ay_entitytype" variant="meta" />
      </template>

      <template #egcs_ay_templatekind-cell="{ row }">
        <UBadge color="neutral" variant="subtle">
          {{ row.original.egcs_ay_templatekind.toUpperCase() }}
        </UBadge>
      </template>

      <template #outputFormats-cell="{ row }">
        <div class="flex flex-wrap gap-1">
          <UBadge v-for="format in row.original.egcs_ay_outputformats" :key="format" color="neutral" variant="subtle">
      ...
      <template #actions-cell="{ row }">
        <div class="flex items-center gap-2">
          <UButton v-if="canUpdate" icon="i-lucide-pencil" color="neutral" variant="ghost" size="sm" class="cursor-default" :aria-label="t('common.edit_named', { name: getTemplateActionTarget(row.original) })" @click="openUpdate(row.original)" />
          <UButton v-if="canDelete" icon="i-lucide-trash" color="error" variant="ghost" size="sm" class="cursor-default" :aria-label="t('common.delete_named', { name: getTemplateActionTarget(row.original) })" @click="deleteTemplate(row.original)" />
        </div>
      </template>
    </CommonResourceLayoutCard>
    ...
    <UModal
      ...
      <template #body>
        <UForm :state="selectedTemplate" :validate="validate" class="space-y-4" @submit="saveTemplate">
          <UFormField :label="t('transfer_payment.entity_type')" name="egcs_ay_entitytype">
            <CommonEnumSelect
              v-model="selectedTemplate.egcs_ay_entitytype"
              name="transfer_payment_document_template_entity_type"
              class="w-full" />
          </UFormField>
          <UFormField :label="t('transfer_payment.document_templates.template_kind')" name="egcs_ay_templatekind">
            <CommonEnumSelect
              v-model="selectedTemplate.egcs_ay_templatekind"
              name="transfer_payment_document_template_kind"
              :disabled="Boolean(selectedTemplate.id)"
              class="w-full" />
          </UFormField>
          <UFormField :label="t('transfer_payment.name_en')" name="egcs_ay_name_en">
            <UInput v-model="selectedTemplate.egcs_ay_name_en" class="w-full" />
          </UFormField>
          <UFormField :label="t('transfer_payment.name_fr')" name="egcs_ay_name_fr">
            <UInput v-model="selectedTemplate.egcs_ay_name_fr" class="w-full" />
          </UFormField>
          <UFormField :label="t('transfer_payment.description_en')" name="egcs_ay_description_en">
            <UTextarea v-model="selectedTemplate.egcs_ay_description_en" class="w-full" />
          </UFormField>
          <UFormField :label="t('transfer_payment.description_fr')" name="egcs_ay_description_fr">
            <UTextarea v-model="selectedTemplate.egcs_ay_description_fr" class="w-full" />
          </UFormField>
          <UFormField :label="t('transfer_payment.document_templates.output_formats')" name="egcs_ay_outputformats" required>
            <UCheckboxGroup
              v-model="selectedTemplate.egcs_ay_outputformats"
          ...
          <UCheckbox v-model="selectedTemplate.egcs_ay_active" :label="t('common.active')" />
